...
def print_diff(str1, str2, hash_type):
    print(f'Diferenças no {hash_type}:')
    for i, (char1, char2) in enumerate(zip(str1, str2)):
        if char1 != char2:
            print(f'Posição {i}: {char1} é diferente de {char2}')

for mensagem, hash_item in zip(mensagens, hashes):
    SHA256 = hashlib.sha256(mensagem.encode('utf-8')).hexdigest()
    MD5 = hashlib.md5(mensagem.encode('utf-8')).hexdigest()

    hash_SHA256, hash_MD5 = hash_item.split(" - ")

    if hash_item == SHA256 + " - " + MD5:
        print(mensagem)
        print("- SHA256 e MD5 corretos\n")
    elif hash_SHA256 != SHA256 and hash_MD5 != MD5:
        
        print(mensagem)
        print("- SHA256 e MD5 incorretos\n")
        print_diff(hash_SHA256, SHA256, 'SHA256')
        print_diff(hash_MD5, MD5, 'MD5')
        print()
    else:
        if hash_MD5 != MD5:
            print(mensagem)
            print("- Somente MD5 incorreto\n")
            print_diff(hash_MD5, MD5, 'MD5')
            print()
        if hash_SHA256 != SHA256:
            print(mensagem)
            print("- Somente SHA256 incorreto\n")
            print_diff(hash_SHA256, SHA256, 'SHA256')
            print()
